...
class Indicators {
  ...
  static ({List<double?> macd, List<double?> signal, List<double?> histogram}) macd(List<Candle> candles, {int fast = 12, int slow = 26, int signalPeriod = 9}) {
    ...
    for (int i = 0; i < macdLine.length; i++) if (macdLine[i] != null) validIndices.add(i);
    if (validIndices.length >= signalPeriod) {
      final k = 2 / (signalPeriod + 1);
      double prev = 0;
      for (int idx = 0; idx < signalPeriod; idx++) prev += macdLine[validIndices[idx]]!;
      prev /= signalPeriod;
    ...
    for (int i = 0; i < candles.length; i++) if (macdLine[i] != null && signalLine[i] != null) histogram[i] = macdLine[i]! - signalLine[i]!;
  ...
  static List<double?> atr(List<Candle> candles, {int period = 14}) {
    ...
    for (int i = 0; i < candles.length; i++) {
      if (i == 0) { trueRanges.add(candles[i].range); continue; }
  ...
  static ({List<double?> mid, List<double?> upper, List<double?> lower}) bollingerBands(List<Candle> candles, {int period = 20, double stdDevMultiplier = 2}) {
  ...
  static double _sqrt(double value) {
    ...
    double x = value;
    double prev;
    do { prev = x; x = (x + value / x) / 2; } while ((x - prev).abs() > 1e-10);
    return x;
  }
