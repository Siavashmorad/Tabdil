...
/// Analysis-only scanner. It never places or modifies an order.
class LiveSignalScanner {
  ...
  Future<LiveSignal?> scanBest({int maxMarkets = 30}) async {
    final markets = await api.getExchangeInfo();
  ...
  Future<LiveSignal?> _analyze(String symbol) async {
    final results = await Future.wait([api.getRecentTrades(symbol, limit: 120), api.getDepth(symbol, limit: 20)]);
